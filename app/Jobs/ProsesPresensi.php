...
class ProsesPresensi implements ShouldQueue
{
     ...
     */
    public function __construct($nik, $tanggal)
    {
        //
        $this->nik = $nik;
        if($tanggal){
    ...
    public function handle(): void
    {
        ...
        if($logFingerPrint){
            ...
            $jamMasuk = Carbon::now()->setTimeFromTimeString($waktuKerja->jam_masuk)->setTimezone('Asia/Jakarta');
            $jamPulang = Carbon::now()->setTimeFromTimeString($waktuKerja->jam_pulang)->setTimezone('Asia/Jakarta');
